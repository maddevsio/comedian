...
func TestDisplayHelpText(t *testing.T) {
	...
	testCase := []struct {
		...
		{"add", "To add members use `add` command. Here is an example: `add @user @user1 / admin` You can add members with _admin, pm, developer, designer_ roles, default is a developer role, if the role is not selected! \n"},
