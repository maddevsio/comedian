...
func (m *MySQL) SelectStandupsFiltered(userID, channelID string, dateStart, dateEnd time.Time) ([]model.Standup, error) {
	items := []model.Standup{}
	err := m.conn.Select(&items, "SELECT * FROM `standups` WHERE channel_id=? AND user_id =? AND created BETWEEN ? AND ?",
...
func (m *MySQL) CreateStandupTime(st int64, channelID string) error {
	res, err := m.conn.Exec("UPDATE `channels` SET channel_standup_time=? WHERE channel_id=?", st, channelID)
	if err != nil {
		return err
	}
	return nil
...
func (m *MySQL) UpdateChannelStandupTime(st int64, channelID string) error {
	...
	var i model.Channel
	err = m.conn.Get(&i, "SELECT * FROM `channels` WHERE id=?", s.ID)
	if err != nil {
		return s, err
	}
	return s, nil
