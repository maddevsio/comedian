...
func prepareTime(timestring string) (date time.Time) {
...
func MakeActiveSprint(collectorInfo CollectorInfo) ActiveSprint {
	...
	startDate := prepareTime(collectorInfo.SprintStart)
	endDate := prepareTime(collectorInfo.SprintEnd)
...
func MakeMessage(bot *bot.Bot, activeSprint ActiveSprint) string {
	...
	sprintDays := localizer.MustLocalize(&i18n.LocalizeConfig{
		DefaultMessage: &i18n.Message{
			...
			Other:       "Sprint lasts {{.totalDays}} d.,{{.passedDays}} d. passed, {{.leftDays}} d. left.",
		},
		TemplateData: map[string]interface{}{
